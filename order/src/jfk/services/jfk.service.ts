...
@Injectable()
export class JfkService implements IJfkService {
  async saveJfkOneway_prod(queryRunner: QueryRunner, orderId: string, jfkOneway: any): Promise<any> {
    try {
      await queryRunner.startTransaction();

      const existingOrderJfkOneway = await queryRunner.manager.findOne(JfkOneway, { where: { orderId: orderId } });
      if (existingOrderJfkOneway) return true;
  ...
  async saveJfkShuttleRt_prod(queryRunner: QueryRunner, orderId: string, jfkShuttleRt: any): Promise<any> {
    try {
      await queryRunner.startTransaction();

      const existingOrderJfkShuttleRt = await queryRunner.manager.findOne(JfkShuttleRt, { where: { orderId: orderId } });
      if (existingOrderJfkShuttleRt) return true;
