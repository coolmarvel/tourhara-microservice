...
export class TourService implements ITourService {
  async saveTour_prod(queryRunner: QueryRunner, orderId: string, tour: any): Promise<any> {
    try {
      await queryRunner.startTransaction();

      const existingOrderTour = await queryRunner.manager.findOne(Tour, { where: { orderId: orderId } });
      if (existingOrderTour) return true;
  ...
  async saveTourInfo_prod(queryRunner: QueryRunner, orderId: string, tourInfo: any): Promise<any> {
    try {
      await queryRunner.startTransaction();

      const existingOrderTourInfo = await queryRunner.manager.findOne(TourInfo, { where: { orderId: orderId } });
      if (existingOrderTourInfo) return true;
