...
export class OrderStagingService implements IOrderService {
   ...
   * Synchronize
   */
  // async synchronizeOrder(pageNumber: number): Promise<any> {
  //   const queryRunner = this.dataSource.createQueryRunner();
  //   await queryRunner.connect();

  //   try {
  //     console.log(`Order migration start for page: ${pageNumber}`);
  //     await queryRunner.startTransaction();

  //     // Fetch orders and process them
  //     const orders = await this.listAllOrders(pageNumber, 10);
  //     if (orders.length === 0) {
  //       await queryRunner.commitTransaction();
  //       console.log(`No more orders to migrate. Completed migration up to page: ${pageNumber}`);
  //       return true;
  //     }
  ...
  //     await queryRunner.commitTransaction();
  //     console.log(`Order migration completed for page: ${pageNumber}`);

  //     // Recursive call for the next page
  //     await this.synchronizeOrder(pageNumber + 1);
  //   } catch (error) {
  //     console.error(`Error occurred during order migration for page: ${pageNumber}`, error);

  //     if (error instanceof TransactionNotStartedError) {
  //       console.log(`Transaction not started error caught for page: ${pageNumber}, retrying...`);
  //       await this.synchronizeOrder(pageNumber);
  //     } else {
  //       await queryRunner.rollbackTransaction();
  //       console.error(`Rollback transaction due to error for page: ${pageNumber}`);
  //     }
  //   } finally {
