    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await queryRunner.manager.query(
          ...
          [checkListId, data.date, data.page, data.per_page, BigInt(data.total), prevResult.length === 0 ? BigInt(data.total) : BigInt(prevResult[0].acc_total) + BigInt(data.total)],
        );
  ...
  async update(queryRunner: QueryRunner, data: any): Promise<any> {
    return new Promise(async (resolve, reject) => {
      try {
        await queryRunner.manager.query(
          `UPDATE \`check_list\` 
            SET page=?,per_page=?,daily_total=?,updated_at=NOW()
            WHERE iso_date=?;`,
          [data.page, data.perPage, data.total, data.date],
        );
