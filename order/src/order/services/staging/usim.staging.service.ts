    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await queryRunner.manager.query(
          ...
          [snapInfoId, snapInfo.mobile_snap === '' ? null : snapInfo.mobile_snap, orderId],
        );
        ...
        await queryRunner.manager.query(
          `INSERT INTO \`h2ousim\` (
            h2ousim_id,usim_extension,eid_number,previous_order_number,order_id,created_at,updated_at
          ) VALUES (?,?,?,?,?,NOW(),NOW());`,
          [
            h2ousimId,
  ...
  async update(queryRunner: QueryRunner, snapInfo: any, usimInfo: any, h2ousim: any, orderId: string): Promise<any> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        console.error('Usim Service Update Error');
        console.error(error);
        return reject(error);
      }
    });
