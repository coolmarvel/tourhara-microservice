...
export default class ProductService implements IProductService {
  ...
  async select(queryRunner: QueryRunner, id: any): Promise<any> {
    try {
      const product = await queryRunner.manager.query(`SELECT * FROM \`product\` WHERE id=?;`, [BigInt(id)]);
      ...
      if (exist.length > 0) {
        await queryRunner.manager.query(
          `UPDATE \`product\` SET
            ...
            date_created_gmt=?, date_modified=?, date_modified_gmt=?, updated_at=NOW()
          WHERE id=?;`,
          [
            product.name,
            ...
            product.date_created_gmt ?? product.date_created_gmt,
            product.date_modified ?? product.date_modified,
            product.date_modified_gmt ?? product.date_modified_gmt,
            BigInt(product.id),
          ],
        );
        logger.info(`Updated product record for product_id=${exist[0].product_id}.`);
      } else {
        await queryRunner.manager.query(
          `INSERT INTO \`product\` (
        ...
        logger.info(`Inserted new product record.`);
      }
    } catch (error: any) {
